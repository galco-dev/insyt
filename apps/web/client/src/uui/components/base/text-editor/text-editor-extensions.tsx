...
const DEFAULT_FONT = { id: "Inter", label: "Inter" } satisfies SelectItemType;
const DEFAULT_FONT_SIZE = { id: "16px", label: "16px" } satisfies SelectItemType;
const DEFAULT_COLOR_SWATCH = "#181D27";

const fonts: SelectItemType[] = [
    DEFAULT_FONT,
...
export const TextEditorColor = ({ className }: { className?: string }) => {
    ...
    const [color, setColor] = useState<Color>(parseColor(DEFAULT_COLOR_SWATCH));
        ...
        <DialogTrigger>
            <Tooltip title="Text color">
                <EditorButton aria-label="Text color" isDisabled={isDisabled} className={className}>
                    <div
                        style={{
                            outlineColor: selectionColor || DEFAULT_COLOR_SWATCH,
                            backgroundColor: selectionColor || DEFAULT_COLOR_SWATCH,
                        }}
                        ...
                        <RadioGroup className="flex flex-col items-start gap-2" value={color?.toString("hex")} onChange={handleColorChange}>
                            ...
                            <div className="flex w-full shrink-0 items-center">
                                ...
                                <ColorField aria-label="Custom color" className="flex min-w-0 flex-1" value={customColor} onChange={handleCustomColorChange}>
                                    <InputBase size="md" wrapperClassName="flex-1 w-23 min-w-0" />
                                </ColorField>
                            </div>
                        </RadioGroup>
                    </div>
...
export const TextEditorFontFamily = ({ className }: { className?: string }) => {
    ...
    const { currentFontFamily } = useEditorState({
        editor,
        selector: ({ editor }) => ({
            currentFontFamily: editor?.getAttributes("textStyle")?.fontFamily || DEFAULT_FONT.id,
        }),
    });
